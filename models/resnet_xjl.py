...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, radix=1, groups=1, bottleneck_width=64,
                 ...
                 final_drop=0.2, dropblock_prob=0,  
...
class ResNet3D(nn.Module):
    ...
    """
    # pylint: disable=unused-variable
    def __init__(self, model_2d,  block, layers, ipt_dim=1, radix=1, groups=1, bottleneck_width=64,
                 num_classes=1000, dilated=False, dilation=1,
                 deep_stem=False, stem_width=64, avg_down=False,
                 rectified_conv=False, rectify_avg=False,
                 avd=False, avd_first=False,
        ...
        conv_kwargs = {'average_mode': rectify_avg} if rectified_conv else {}
        new_conv1 = nn.Conv2d(ipt_dim, 32, kernel_size=7, stride=2, padding=3, bias=False)
        if deep_stem:
            self.conv1 = nn.Sequential(
        ...
        # self.neck = nn.Linear(512 * block.expansion, 256)
    ...
    def forward(self, x):
        ...
        # feat_map = self.neck(x)

        x = self.fc(x)

        return feat_map,features,x
